...
class ModelModuleBrainyFilter extends Model 
{
    ...
    public function editLayoutModules($layoutsData)
    {
        $sql = new SqlStatement();
        $sql->delete()->from(array('layout_module'))->where('code LIKE "brainyfilter.%"');
        $this->db->query($sql);
        $sql->clean()->insertInto('layout_module', $layoutsData);
        $this->db->query($sql);
    }
    ...
    public function getEnabledLayoutModules()
    {
        ...
        if ($res->num_rows) {
            foreach($res->rows as $row) {
                ...
                $moduleSettings = $this->model_extension_module->getModule($moduleId);
                $output[] = (int)$moduleSettings['bf_layout_id'];
            }
        }
        return $output;
    }
