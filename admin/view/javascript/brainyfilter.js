...
var BrainyFilterAdm = {
    ...
    init : function() {
        ...
        if (this.currentModule !== false) {
            this.selectTab(jQuery('#bf-adm-main-menu .tab').eq(1));
            jQuery('#bf-layout-selector').val(this.currentModule);
...
//                console.log(name + ' - not found');
    ...
    initTabs : function(parent) {
        parent.find('.tab.selected').each(function(){
            BrainyFilterAdm.selectTab(jQuery(this));
        });
    ...
    submitForm : function() {
        if (BrainyFilterAdm.isFirstLaunch) {
            jQuery.post(BrainyFilterAdm.modRefreshActionUrl, {});
        }
    ...
    attrValues : {
        init : function() {
            ...
            popup.find('.htabs .tab').first().addClass('selected');
