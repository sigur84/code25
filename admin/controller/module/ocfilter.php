...
class ControllerModuleOCFilter extends Controller {
    ...
    $data['sitemap_link'] = HTTP_CATALOG . 'index.php?route=feed/ocfilter_sitemap';
		...
		if (isset($this->request->post['ocfilter_stock_status_type'])) {
			$data['ocfilter_stock_status_type'] = $this->request->post['ocfilter_stock_status_type'];
		} else {
			$data['ocfilter_stock_status_type'] = $this->config->get('ocfilter_stock_status_type');
		}
