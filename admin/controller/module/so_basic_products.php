...
class ControllerModuleSobasicproducts extends Controller {
	...
	public function index() {
		...
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('module/so_basic_products', 'token=' . $this->session->data['token'], 'SSL'));
		}
		// Get module id new 
		$moduleid_new= $this->model_module_so_basic_products->getModuleId(); // Get module id
		$module_id = '';
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('module/so_basic_products', 'module_id='.$module_id.'&token=' . $this->session->data['token'], 'SSL'));
		...
		$data['link'] = $this->url->link('module/so_basic_products', 'token=' . $this->session->data['token'] . '', 'SSL');
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_module_so_basic_products->getCategories($filter_data);
