...
class ControllerModuleBrainyFilter extends Controller {
    ...
    public function index()
    {
        ...
        if (isset($this->request->post['bf'])) {
            ...
            if ($this->_validate($post)) {
                ...
                if ($this->request->post['action'] == 'apply') {
                    $this->response->redirect($this->url->link('module/brainyfilter', 'token=' . $this->session->data['token'], 'SSL'));
                } else {
        ...
        if (isset($this->request->get['module_id'])) {
            $curModule = $this->model_extension_module->getModule($this->request->get['module_id']);
		...
		$this->response->setOutput($this->load->view('module/brainyfilter.tpl', $this->_data));
    }
    ...
    private function _saveSettings($data) 
    {
        ...
        $this->model_extension_module->deleteModulesByCode('brainyfilter');
        ...
        foreach ($data as $id => $set) {
            ...
            $settings['brainyfilter_layout_' . $id] = $set;
        }
        
        $this->model_setting_setting->editSetting('brainyfilter', $settings);
        
        $this->model_module_brainyfilter->editLayoutModules($lModules);
    ...
    private function _applySettings() 
    {
        if (isset($this->request->post['bf']) && is_array($this->request->post['bf'])) {
            ...
            $i = 0;
            $enabledModules = $this->model_module_brainyfilter->getEnabledLayoutModules();
            ...
            while ($set = $this->config->get('brainyfilter_layout_' . $i)) {
                ...
                if ($i && !in_array($i, $enabledModules)) {
                    $settings[$i]['layout_enabled'] = 0;
                }
                $i ++;
            }
            if (!empty($settings)) {
