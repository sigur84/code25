...
class ControllerModuleSoMegaMenu extends Controller {
    ...
    public function index() {
        ...
        $this->document->setTitle('So_megamenu');
        ...
        if(isset($_GET['delete'])) {
            if($this->validate()){
                if($this->model_module_so_megamenu->deleteMenu(intval($_GET['delete']))) {
                    $this->session->data['success'] = 'This menu has been properly removed from the database.';
                } else {
                    $this->session->data['error_warning'] = $this->model_module_so_megamenu->displayError();
                }
            } else {
                $this->session->data['error_warning'] = $this->language->get('error_permission');
            }
            $this->response->redirect(HTTPS_SERVER . 'index.php?route=module/so_megamenu&token=' . $this->session->data['token'].'&action=create&module_id='.$this->request->get['module_id']);
        }
            ...
            if(isset($_POST['button-create'])) {
                if($this->validate()) {
                    ...
                    if($error == true) {
                        ...
                        $this->model_module_so_megamenu->addMenu($this->request->post);
                ...
                $this->response->redirect(HTTPS_SERVER . 'index.php?route=module/so_megamenu&token=' . $this->session->data['token']);
            }
        }		
        if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
            if(isset($_POST['button-back'])) {
				$this->response->redirect(HTTPS_SERVER . 'index.php?route=module/so_megamenu&token=' . $this->session->data['token'].'&module_id='.$this->request->get['module_id']);
			}	
            ...
            elseif(isset($_POST['button-edit'])) {
                if($this->validate()) {
                    ...
                    if($error == true) {
						...
						if($this->request->post['link'])
							$this->request->post['link'] = serialize ($this->request->post['link']);
                        $this->model_module_so_megamenu->saveMenu($this->request->post);
                        $this->session->data['success'] = $this->language->get('text_success');
						$this->response->redirect(HTTPS_SERVER . 'index.php?route=module/so_megamenu&token=' . $this->session->data['token'].'&edit='.$this->request->post['id'].'&module_id='.$this->request->get['module_id']);
                    }
                } else {
                    $this->session->data['error_warning'] = $this->language->get('error_permission');
                }
        ...
        $data['nestable_list'] = $this->model_module_so_megamenu->generate_nestable_list($data['language_id']);
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/so_megamenu', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('module/so_megamenu', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}	
        ...
        if (!isset($this->request->get['module_id'])) {
            $data['action'] = $this->url->link('module/so_megamenu', 'token=' . $this->session->data['token'], 'SSL');
        } else {
            $data['action'] = $this->url->link('module/so_megamenu', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
        }
        ...
        if (isset($_GET['jsonstring'])) {
            if($this->validate()){
                ...
                foreach ($readbleArray as $key => $value) {
                    if (is_array($value)) {
                        $this->model_module_so_megamenu->save_rang($value['parentID'], $value['id'], $key);
                    }
                }
        ...
        if(isset($_GET['edit'])) {
            ...
            $dane = $this->model_module_so_megamenu->getMenu(intval($_GET['edit']));		
			$this->load->model('tool/image');
            ...
            if($dane) {
                ...
                $data['list_categories'] = $this->model_module_so_megamenu->getCategories($dane['content']['categories']['categories']);
            } else {
        ...
        {
            $dane = $this->model_module_so_megamenu->getMenu(intval($_GET['duplicate']));
            $id_parent_add = $this->model_module_so_megamenu->addMenu($dane);
            $subcategories = $this->model_module_so_megamenu->getSubMenu(intval($_GET['duplicate']));
            foreach ($subcategories as $result) {
                $data = array(
                    ...
                    'list_categories' => $this->model_module_so_megamenu->getCategories(unserialize($result['content']['categories']['categories']))
                );
                $this->model_module_so_megamenu->addMenu($data);
            }
            $this->response->redirect(HTTPS_SERVER . 'index.php?route=module/so_megamenu&token=' . $this->session->data['token'].'&action=create&module_id='.$this->request->get['module_id']);
        }
        elseif (isset($_GET['changestatus']))
        {
            $dane = $this->model_module_so_megamenu->getMenu(intval($_GET['changestatus']));
            if ($dane['status']==1)
                $status['status'] =0;
            else
                $status['status'] =1;
            $status['id'] = intval($_GET['changestatus']);
            $this->model_module_so_megamenu->UpdatePosition($status);
            $this->response->redirect(HTTPS_SERVER . 'index.php?route=module/so_megamenu&token=' . $this->session->data['token'].'&action=create&module_id='.$this->request->get['module_id']);
        }
        $this->load->model('tool/image');
        if (isset($this->request->post['icon']) && is_file(DIR_IMAGE . $this->request->post['icon'])) {
            $data['icon'] = $this->model_tool_image->resize($this->request->post['icon'], 100, 100);
        } elseif (!empty($dane) && is_file(DIR_IMAGE . $dane['icon'])) {
            $data['icon'] = $this->model_tool_image->resize($dane['icon'], 100, 100);
        } else {
        ...
        $this->response->setOutput($this->load->view('module/so_megamenu.tpl', $data));
    }

    public function uninstall() {
        $this->load->model('setting/setting');
        $this->load->model('module/so_megamenu');
        $this->model_module_so_megamenu->uninstall();
    }
    ...
    public function install() {
		...
		if(is_array($new_module))		
			$this->model_module_so_megamenu->install($new_module['module_id']);
    }
    ...
    protected function validate() {
        if (!$this->user->hasPermission('modify', 'module/so_megamenu')) {
