...
class ControllerModuleSolistingtabs extends Controller {
	...
	// Delete Module
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('module/so_listing_tabs', 'token=' . $this->session->data['token'], 'SSL'));
		}
	// Get Module Id new 
		$moduleid_new= $this->model_module_so_listing_tabs->getModuleId();
		$module_id = '';	
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('module/so_listing_tabs', 'module_id='.$module_id.'&token=' . $this->session->data['token'], 'SSL'));
		...
		$moduleid_new= $this->model_module_so_listing_tabs->getModuleId(); // Get module id
		...
		$data['link'] = $this->url->link('module/so_listing_tabs', 'token=' . $this->session->data['token'] . '', 'SSL');
		//--------------------------------Load Data -------------------------------------------
	...
	public function autocomplete_category() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$results = $this->model_module_so_listing_tabs->getCategories($filter_data);
