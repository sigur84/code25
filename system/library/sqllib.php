        ...
        if (is_array($params)) {
            if (count($params)) {
                foreach ($params as $param) {
                    $condition = preg_replace('/\?/', "'" . self::escape($param) . "'", $condition, 1);
                }
            }
        } else {
            $condition = preg_replace('/\?/', "'" . self::escape($params) . "'", $condition, 1);
        }
        array_push($this->_where, $condition);
        return $this;
    }
    
    public function in($column, $data = array()) {
        if (!count($data)) {
            return $this;
        }
        $preparedData = array();
        foreach ($data as $item) {
            $preparedData[] = self::escape($item);
        }
        $this->where($column . ' IN (' . implode(',', $preparedData) . ')');
        return $this;
    }
    /**
    ...
    public function multipleWhere($conditions, $operator = 'OR')
    {
        if (count($conditions)) {
            ...
            foreach ($conditions as $item) {
                if (is_array($item)) {
                    if (count($item) > 1) {
                        $conArr[] = preg_replace('/\?/', "'" . self::escape($item[1]) . "'", $item[0], 1);
                    } else {
                        $conArr[] = $item[0];
                    }
                } else {
