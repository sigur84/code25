...
class ControllerModuleSolistingtabs extends Controller {
	...
	public function index($setting) {
		...
		if($is_ajax && isset($_POST['is_ajax_listing_tabs']) && $_POST['is_ajax_listing_tabs']){
			...
			$display_banner_image 		= $setting['display_banner_image'];
...

	{
...

		{
...

			{
				...
				$name = (($setting['title_maxlength'] != 0 && strlen($product_info['name']) > $setting['title_maxlength']) ? utf8_substr(strip_tags(html_entity_decode($product_info['name'], ENT_QUOTES, 'UTF-8')), 0, $setting['title_maxlength']) . '250' : $product_info['name']);
