...
class ControllerModuleSoextraslider extends Controller {
	...
	public function index($setting) {
			...
			$data['sale_text'] = $this->language->get('sale_text');
...

		{
			...
			$name = ((strlen($product_info['name']) > $setting['title_maxlength'] && $setting['title_maxlength'] !=0)  ? utf8_substr(strip_tags(html_entity_decode($product_info['name'], ENT_QUOTES, 'UTF-8')), 0, $setting['title_maxlength']) . '250' : $product_info['name']);
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/so_extra_slider/so_extra_slider.tpl')) {

			return $this->load->view($this->config->get('config_template') . '/template/module/so_extra_slider/so_extra_slider.tpl', $data);

		} else {

			return $this->load->view('default/template/module/so_extra_slider/so_extra_slider.tpl', $data);
