...
//        echo $sql;
    ...
    private function _prepareSearchConditions()
    {
        ...
        if (!empty($this->searchTagString)) {
            $search[] = array('pd.tag LIKE "%' . $this->searchTagString . '%"');
        }
        if (!empty($this->searchDescriptionString)) {
            $search[] = array('pd.description LIKE "%' . $this->searchDescriptionString . '%"');
        }
...
//            ->distinct()
	...
	public function getOptions()
	{
        ...
        $sql->select($columns)
            ...
            ->order(array('ov.sort_order', 'o.sort_order', 'od.name', 'ovd.name')); 
