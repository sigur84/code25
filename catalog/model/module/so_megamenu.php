<?php
class ModelModuleSomegamenu extends Model {
    ...
    public function getSubmenu($id) {
		...
		foreach ($query->rows as $row) {
            ...
            if(isset($content['subcategory'])) {
				$subcategory = array();
				$limit_level1 = (isset($content['subcategory']['limit_level_1']) && $content['subcategory']['limit_level_1'] ) ? $content['subcategory']['limit_level_1'] :4;
				$limit_level2 = (isset($content['subcategory']['limit_level_2']) && $content['subcategory']['limit_level_2'] ) ? $content['subcategory']['limit_level_2'] :4;
				
				$categories_info = $this->model_catalog_category->getCategory($content['subcategory']['category']);
				...
				$subcategory['name'] = (isset($categories_info['name'])) ? $categories_info['name'] : '';
				$subcategory['href']  = $this->url->link('product/category', 'path=' . $content['subcategory']['category']);
				
				$this->load->model('tool/image');
				$image = empty($categories_info['image']) ? 'no_image.png' : $categories_info['image'];
				$subcategory['thumb']	= $this->model_tool_image->resize($image, 100, 100);
				...
				$child_categories = $this->getCategoriesByCatId($content['subcategory']['category'],$limit_level1);
				...
				if(is_array($data_categories)) {
                    $subcategory['categories'] = $this->getCategories($data_categories);
                } else {
                    $subcategory['categories'] = false;
                }
