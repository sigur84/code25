	...
	if (!empty($child_items)) {
		...
		foreach ($child_items as $product) {
			...
			<?php } ?>
				...
				<div class="item-inner item-inner-dev product-thumb transition">
					<?php if($product_image){ ?>
						 <div class="image image-dev">
							...
							<ul class="list-button-dev">
							  ...
							  <li>
								<button type="button" data-toggle="tooltip" title="<?php echo $button_compare; ?>" onclick="compare.add('<?php echo $product['product_id']; ?>');"><i class="fa fa-refresh"></i></button>
							  </li>
							  <?php }?>
							</ul>
						</div>
				...
				<?php if($display_title || $display_description){?>
					<div class="caption caption-dev">
					  <?php 
						
					  if (isset($product['rating'])) { ?>
							<div class="rating rating-dev">
							  <?php for ($j = 1; $j <= 5; $j++) { ?>
						...
						<?php } ?>
							...
							<?php if($display_add_to_cart){?>
								<button class="add-cart-dev" type="button" onclick="cart.add('<?php echo $product['product_id']; ?>');"> <span><?php echo $button_cart; ?></span></button>
							<?php }?>
					</div>
				<?php }?>
...
<script type="text/javascript">
	jQuery(document).ready(function($){
		...
		nb_column4 = <?php echo $nb_column4; ?>;
		 ...
		 var $navpage = $(".ltabs-tabs-wrap", $tag_id);
