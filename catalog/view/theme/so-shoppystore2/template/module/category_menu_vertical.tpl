...
<div id="so_vertical_menu" class="so-vertical-menu clearfix compact-hidden">
	...
	<ul class="vf-menu clearfix menu-content">
		<?php foreach ($categories as $category) { ?>
			...
			<?php }else{ ?>
				<li class="<?php if ($category['children']){ echo"level1 sovm-havechild";} ?>"><a href="<?php echo $category['href']; ?>"><span><?php echo $category['name']; ?></span></a>
			<?php } ?>
			<?php if ($category['children']) { echo '<span class="vf-button icon-close"></span>'; ?>
				<ul class="sovm-menu level2">
				<?php for ($i = 0; $i < count($category['children']);) { ?>
					...
					<?php for (; $i < $j; $i++) { ?>
						<?php if (isset($category['children'][$i])) { ?>
							...
							<?php }else{ ?>
								<li class="<?php if ($category['children'][$i]['subchildren']){ echo"level2 sovm-havechild";} ?>"><a href="<?php echo $category['children'][$i]['href']; ?>"><span><?php echo $category['children'][$i]['name']; ?></span></a>
							<?php } ?>

							<?php if ($category['children'][$i]['subchildren']) { ?>
								
								<?php for ($l = 0; $l < count($category['children'][$i]['subchildren']);) { ?>
									<ul class="sovm-menu level3">
									...
									<?php for (; $l < $m; $l++) { ?>
										<?php if (isset($category['children'][$i]['subchildren'][$l])) { ?>
											...
											<?php }else{ ?>
												<li class="<?php if ($category['children'][$i]['subchildren'][$l]){ echo"level3";} ?>"><a href="<?php echo $category['children'][$i]['subchildren'][$l]['href']; ?>"><span><?php echo $category['children'][$i]['subchildren'][$l]['name']; ?></span></a>
											<?php } ?>
										<?php } ?>
									<?php } ?>
									</ul>
