    ...
    if (event = e.match(/iPad/i) ? "touchstart" : "click", widthC = $("#sp-cpanel").width() + 40, $("#sp-cpanel_btn").bind("click", function() {
        ...
        $.each(t, function(e, t) {
            switch (addValue = params[t], t) {
                case "scheme":
                    $.cookie("customColorScheme", addValue), changeColor(addValue);
                    break;
                case "layoutbox":
                    $.cookie("layoutbox", addValue), changeLayoutBox(addValue);
                    break;
                case "pattern":
                    $.cookie("bgPattern", addValue), changePattern(addValue)
            }
        })
    }
