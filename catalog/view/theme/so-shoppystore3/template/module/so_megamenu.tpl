...
<div class="responsive megamenu-style-dev">
	...
	<div class=" container-megamenu <?php if($ustawienia['full_width'] != '1') { echo 'container'; } ?> <?php if($ustawienia['orientation'] == 1) { echo 'vertical dropdown'; } else { echo 'horizontal'; } ?>">
		<?php if($ustawienia['orientation'] == 1) { ?>
		<div id="menuHeading" class="dropdown-toggle" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
				...
				<div class="container">
					...
					data-transition="<?php if($ustawienia['animation'] != '') { echo $ustawienia['animation']; } else { echo 'none'; } ?>" data-animationtime="<?php if($ustawienia['animation_time'] > 0 && $ustawienia['animation_time'] < 5000) { echo $ustawienia['animation_time']; } else { echo 500; } ?>">
											...
											elseif($submenu['content_type'] == '6') {
													if($submenu['productlist']['show_title'] == 1)
														...
														if(isset($submenu['productlist']['products']) && $submenu['productlist']['products']){
															...
															echo '<div class="col-xs-'.$itempage.'">';
						...
						echo "<li class='".$class."' ".$title."><p class='close-menu'></p>";						
							if(is_array($row['submenu']) && !empty($row['submenu']))
								echo "<a href='".$row['link']."' class='".$class_link."' ".$target."><strong>".$row['icon'].$row['name'][$lang_id].$row['description']."</strong> <span class='label'> ".$row['label_item']."</span> <b class='caret' ></b> </a>";
							else
								echo "<a href='".$row['link']."' class='".$class_link."' ".$target."><strong>".$row['icon'].$row['name'][$lang_id].$row['description']."</strong> <span class='label'> ".$row['label_item']."</span></a>";
										...
										echo '<div class="col-sm-'.$submenu['content_width'].'">';
												...
												elseif($submenu['content_type'] == '5') {
													if($submenu['subcategory']['show_title'] == 1)
														echo'<a href="'.$submenu['subcategory']['href'].'" class="title-submenu">'.$submenu['subcategory']['name'].'</a>';	
												...
												elseif($submenu['content_type'] == '6') {
													if($submenu['productlist']['show_title'] == 1)
														...
														if(isset($submenu['productlist']['products']) && $submenu['productlist']['products']){
															foreach ($submenu['productlist']['products'] as $product) {
															$itempage = (isset($submenu['productlist']['col']) && $submenu['productlist']['col']) ? 12/$submenu['productlist']['col'] : 4;
															echo '<div class="col-sm-'.$itempage.'">';
																echo '<div class="product-thumb">';
																	echo '<div class="image">
																		<a href="'.$product['href'].'">
																			<img src="'. $product['thumb'].'" alt="'. $product['name'].'" title="'. $product['name'].'" class="img-responsive" />
