...
<div class="container">
    ...
    <div id="content" class="<?php echo $class; ?>"><?php echo $content_top; ?>
        ...
        <div class="products-category">
			<?php if (isset($soconfig_pages["lsttitle_cate_status"]) && $soconfig_pages["lsttitle_cate_status"] ): ?> <h2 class="title-category"><?php echo $heading_title; ?></h2> <?php endif; ?>
			<?php if ($thumb || $description) { ?>
			  <div class="row">
					<?php if (isset($soconfig_pages["lstimg_cate_status"]) && $soconfig_pages["lstimg_cate_status"] == 1){ ;?>
						<?php if ($thumb) { ?>
						<div class="col-sm-12 form-group"><img src="<?php echo $thumb; ?>" alt="<?php echo $heading_title; ?>" title="<?php echo $heading_title; ?>" class="img-thumbnail" /></div>
						<?php } ?>
					<?php } ; ?>
				<?php if ($description) { ?>
			...
			<?php if ($soconfig_pages["product_catalog_refine"] !='2') { ?>
			<div class="refine-search form-group">
			<div class="row">
				<h3 class="col-xs-12"><?php echo $text_refine; ?></h3>
				<?php
				$device_refine ='form-group ';
				foreach ($devices as $refine => $device) {$device_refine .=  'col-'.$refine.'-'.ceil(12/$soconfig_pages["product_catalog_refine_col_".$refine] ).' ';}
				foreach ($categories as $category) {
					 if(isset($category['thumb'])&& $category['thumb'] != '') { 
						$image = 'image/'.$category['thumb']; 
					 } else { 
						$image = 'image/placeholder.png'; 
					 } 
					 ?>
					<div class="<?php echo $device_refine;?>">
					 <?php if ($soconfig_pages["product_catalog_refine"] =='0'){?>
						  <a href="<?php echo $category['href']; ?>" class="thumbnail"><img src="<?php echo $image; ?>" alt="<?php echo $category['name']; ?>" /> </a>
			...
			<div class="product-filter filters-panel">
			  <div class="row">
				<div class="col-md-2 hidden-sm hidden-xs">
					<div class="view-mode">
						<div class="list-view">
							<button class="btn btn-default grid <?php if($soconfig_pages['product_catalog_mode'] == '0') { echo 'active'; } ?>" data-view="grid" data-toggle="tooltip" title="<?php echo $button_grid; ?>"><i class="fa fa-th"></i></button>
							<button class="btn btn-default list <?php if($soconfig_pages['product_catalog_mode'] == '1') { echo 'active'; } ?>" data-view="list" data-toggle="tooltip" title="<?php echo $button_list; ?>"><i class="fa fa-th-list"></i></button>
						</div>
					</div>
				</div>
  ...
  <!--end content-->
		...
		$(document).ready(function () {
			...
			if($.cookie('display')){
				...
				view = '<?php echo isset($soconfig_pages['product_catalog_mode']) && $soconfig_pages['product_catalog_mode'] ? 'list' : 'grid'?>' ;
			}
